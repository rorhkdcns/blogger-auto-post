...
def generate_blog_content(target_keyword):
    ...
    prompt = (
        ...
        "자극적인 낚시성 문구 대신, 유저들이 실제로 구글이나 네이버에 검색할 법한 핵심 키워드와 구체적인 해결책을 조합하라.\n\n"
        "2. [에버그린 스토리텔링 및 설득 흐름]:\n"
        "   - 실시간 뉴스 요약이나 일시적인 시황 나열을 절대 금지한다. 수개월, 수년 뒤에 읽어도 가치가 있는 지속 가능한 지식과 원리를 논리적으로 설명하라.\n"
        "   - 유저가 이 주제를 검색한 근본적인 이유(돈을 잃을지 모른다는 불안감, 계산의 복잡함 등)에 깊이 공감하고, 이를 해결할 수 있는 객관적인 가이드라인을 제시하라.\n"
        "   - 본문 흐름 속에서 유저가 스스로 자신의 포트폴리오 리스크를 점검하도록 자연스럽게 유도하되, 주식 계산기를 강제로 쓰라는 식의 노골적인 광고 문구는 배제하라.\n"
        "   - 절대 금지어: 본문 그 어디에도 '파소나', 'PASONA', '카피라이팅', 'AI', '인공지능', '자동화', '프로그램', '단계별 전략'이라는 단어는 절대 사용하지 마라.\n\n"
        "3. [자연스러운 가독성 법칙]: 사람이 직접 고심하여 쓴 고품질 블로그처럼 전문적이면서도 친근한 어조(예: ~입니다, ~시점입니다)를 유지하라. "
        "문장은 모바일 화면에서 잘 읽히도록 2~3문장 단위로 흐름을 나누되, 문맥의 연결성을 부드럽게 확보하라. 본문에 대괄호([])나 불필요한 이모지, 특수기호는 배제하라.\n\n"
        "4. [강조 법칙]: 글 전체에서 가장 핵심이 되는 개념어나 수치 딱 2~3개에만 구글 표준 <b><font color=\"#e11d48\">중요데이터</font></b> 양식을 적용하라. "
        "남발하면 스팸 문서로 오인받으므로 극도로 절제하라.\n\n"
        "5. [파트 구성]: 본문은 구조적 완성도를 위해 3개의 파트로 명확히 나누고, 정보의 가치가 높은 소제목을 부여하라.\n"
        "   - 1단계 (개념과 원인): 이 주제나 개념이 주식 투자에서 왜 중요하며, 투자자들이 흔히 저지르는 실수는 무엇인지 분석하라.\n"
        "   - 2단계 (실전 위협과 분석): 이 리스크나 현상이 개인 투자자의 자산과 포트폴리오 평단가에 미치는 실질적인 위협 및 계산의 필요성을 서술하라.\n"
        "   - 3단계 (대응 가이드와 마음가짐): 리스크를 방어하고 현명하게 자산을 배분하기 위해 투자자가 취해야 할 객관적인 기준과 실천 방향을 제안하라.\n\n"
        ...
        "[SUB_TITLE_1]: 소제목1\n"
        "[BODY_1]: 내용1 (강조 양식은 문맥상 꼭 필요한 곳에만 딱 1번 적용)\n"
        "[SUB_TITLE_2]: 소제목2\n"
        "[BODY_2]: 내용2 (강조 양식은 문맥상 꼭 필요한 곳에만 딱 1번 적용)\n"
        "[SUB_TITLE_3]: 소제목3\n"
        "[BODY_3]: 내용3 (강조 양식은 문맥상 꼭 필요한 곳에만 딱 1번 적용)"
    )
